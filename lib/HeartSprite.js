HeartSprite = function(position, clickBehavior) {
    ...
    var driftSpeed;
    var time = 0.0;
    var resetCountdown = 30.0 + (Math.random() * 10);
    ...
    this.init = function() {
        ...
        map1 = textureLoader.load("spriteRev2.png");
    ...
    this.update = function(delta) {
        ...
        if (this.sprite.material.opacity == 0.0) {
            resetCountdown = 30.0 + (Math.random() * 10);
...
HeartSprite.prototype.ResetSprite = function() {
        ...
        this.sprite.position.z = (camera.position.z - Math.random()*30 + 15);


        TweenLite.to(this.sprite.scale, 5.0, {x: "3.0", y: "3.0"});
        TweenLite.to(this.sprite.material, 1.0, {opacity: this.resetOpacity});
